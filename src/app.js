  ...
  const votesHtml = (!submitted && isLoggedIn && nearby && !onCooldown) ? `
    <div style="display:grid;grid-template-columns:1fr 1fr;gap:6px;margin-bottom:10px;">
      ${voteOptions.map(({idx,label,pts,color}) => {
        ...
        return `<button class="pp-vote-btn" data-idx="${idx}" style="border:${isSel ? `2px solid ${color}` : (T ? `1.5px solid ${T.btnBorder}` : '1.5px solid #E2E8F0')};border-radius:10px;background:${isSel ? color : (T ? T.btnBg : '#fff')};padding:9px 8px;cursor:pointer;display:flex;align-items:center;gap:6px;font-size:11px;font-weight:700;color:${isSel ? '#fff' : (T ? T.text : '#fff')};text-align:left;font-family:inherit;transition:all 0.18s cubic-bezier(0.34,1.56,0.64,1);transform:${isSel ? 'translateY(-1px)' : 'none'};box-shadow:${isSel ? `0 4px 12px ${color}44` : 'none'};">
